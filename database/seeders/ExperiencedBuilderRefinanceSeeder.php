...
class ExperiencedBuilderRefinanceSeeder extends Seeder
{
    /**
     * Run the database seeds.
     * This seeder duplicates all New Construction EXPERIENCED BUILDER Purchase loan rules for Refinance transaction types
     */
    public function run(): void
    {
        ...
        // Get all existing Purchase loan rules for New Construction EXPERIENCED BUILDER
        ...
        // Filter only New Construction EXPERIENCED BUILDER loan rules
        $experiencedBuilderPurchaseRules = $purchaseRules->filter(function ($rule) {
            return $rule->experience->loanType
                && $rule->experience->loanType->name === 'New Construction'
                && $rule->experience->loanType->loan_program === 'EXPERIENCED BUILDER';
        });

        if ($experiencedBuilderPurchaseRules->isEmpty()) {
            $this->command->warn('No New Construction EXPERIENCED BUILDER Purchase loan rules found. Please seed EXPERIENCED BUILDER Purchase rules first.');
            return;
        }

        $this->command->info('Found ' . $experiencedBuilderPurchaseRules->count() . ' New Construction EXPERIENCED BUILDER Purchase rules to duplicate.');
        ...
        foreach ($refinanceTypes as $refinanceType) {
            ...
            foreach ($experiencedBuilderPurchaseRules as $purchaseRule) {
        ...
        $this->command->info('New Construction EXPERIENCED BUILDER refinance loan rules seeding completed!');
    }
}